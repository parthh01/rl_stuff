...
class BaseNetworkClass(Model):
    ...
    def save_model(self):
        dirpath = os.path.join(self.MODEL_DIR,self.model_name)
...
class ActorNetwork(BaseNetworkClass):
    ...
    def __init__(self,model_name,num_actions, hidden_layers: List = [128],params = {'lr': 0.1}):
        ...
        self.target_network = self.build_network(num_actions,hidden_layers,target=True)
    ...
    def build_network(self,num_actions,hidden_layers: List = [128],target: bool = False):
        ...
        for l in hidden_layers:
            layers.append(Dense(l,activation = 'tanh',trainable = not target,kernel_initializer = self.kernel_initializer))
        layers.append(Dense(num_actions,activation='tanh',trainable = not target,kernel_initializer = self.kernel_initializer))
        return layers
...
class CriticNetwork(BaseNetworkClass):
    ...
    def __init__(
        ...
        self.target_network = self.build_network(hidden_layers,target=True)
        self.sync_target_networks()

        self.optimizer = Adam(learning_rate=self.params['lr'])
        self.loss_fn = tf.keras.losses.Huber(reduction=tf.keras.losses.Reduction.SUM)
    
    def build_network(self,hidden_layers,target: bool = False):
        layers = [Concatenate(axis=-1,trainable = not target)] 
        for l in hidden_layers:
            layers.append(Dense(l,activation = 'tanh',trainable = not target,kernel_initializer = self.kernel_initializer))
        layers.append(Dense(1,trainable = not target,kernel_initializer = self.kernel_initializer))
        return layers
