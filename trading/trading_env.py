...
def adjusted_sharpe_ratio(rx,rf,sd,min_sharpe=1): # todo: switch to sortino ratio
    return ((rx-rf)/(1+sd)) - min_sharpe # 1+ sd to avoid divide by zero 
...
def create_env(df):
    env = gym.make("TradingEnv",
        ...
        positions =  [i for i in np.arange(-1,1.1,0.1)],#[ -1, 0, 1], # -1 (=SHORT), 0(=OUT), +1 (=LONG)
